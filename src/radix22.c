...
void radix22_DIF(complex *input, complex *output, complex *twids, int samples)
{
...
void butterfly(complex *output, complex *twids, int n, int local_size, int samples)
{
    ...
    output[_n + 0 * local_size / 4] = stage[2][0];
    output[_n + 1 * local_size / 4] = stage[2][1] * twids[1 * twidindex];
    output[_n + 2 * local_size / 4] = stage[2][2] * twids[2 * twidindex];
    output[_n + 3 * local_size / 4] = stage[2][3] * twids[3 * twidindex];
}
